...
. ${PO_PATH}/proj/sw/bash_tools/poshd/lib/object.sh
. ${PO_PATH}/proj/sw/bash_tools/poshd/lib/time.sh
      ...
      disk)
        if [[ -d "${current_object["content"]}" ]]; then
          if [[ "${current_object["action"]}" == "backup" ]]; then
            ...
            rsync -rlpt -uHE --progress --exclude=".hidden_proj/" "${current_object["content"]}" "$PO_PATH"
